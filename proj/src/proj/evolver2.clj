...
(defn make-individual
  ...
  (let [weights (get-weights)
        x (repeatedly 100 #(calculatePortfolioReturns
                            ...
                            totalCash
                            weights))
        avreturns (/ (reduce + x) 100)]
    (print avreturns)
    {:genome weights :port-params port
     :avg-return avreturns
     :risk (/ (standard-deviation x) avreturns)
...
(def cases [:avg-return :risk :diversity :social :avg-return :avg-return])
  ...
  (if (empty? casein) (rand-nth pop)
      (let [cases (remove-last :avg-return (shuffle casein))
            ...
            nextset (if (or (= thiscase ":social") (= thiscase ":avg-return"))
                      (reverse (sort-by thiscase (if (map? pop) (list pop) pop)))
                      (sort-by thiscase (if (map? pop) (list pop) pop)))]
        (loop [x nextset
               bestport (first x)
...
(defn crossover
  ...
  (let [weights1 (:genome w1)
    ...
    (list {:genome normalized-weights1
           ...
           :avg-return (/ (reduce + x1) 100)
           :risk (/ (standard-deviation x1) (/ (reduce + x1) 100))
          ...
          {:genome normalized-weights2
           ...
           :risk (/ (standard-deviation x2) (/ (reduce + x2) 100))
...
(defn evolve
  ...
  (loop [generation 0
    ...
    (let [best (first (reverse (sort-by :avg-return population)))
          ...
          (recur
           ...
           (concat (crossover (second better-half)
                   ...
                   (map #(mutate % totalcash)
                        (concat (crossover (rand-nth better-half) best totalcash)
                                (concat  better-half (list best)))))))))))


;;;---Run evolution with new portfolio parameters
(evolve (take  10 (repeatedly #(make-individual (genp createPortfolio2) 10000))) 40 10000)
